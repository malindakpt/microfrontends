...
//TODO: Add tests
export const dateToBeInRange = (
  received: string | Date,
  min: Date,
  max?: Date,
): void => {
  const parsedDate = new Date(received).getTime();
  const dateBefore = min.getTime();
  const dateAfter = (max ?? new Date()).getTime();
  expect(parsedDate).toBeGreaterThanOrEqual(dateBefore);
  expect(parsedDate).toBeLessThanOrEqual(dateAfter);
};
...
export const testIsNullOrWhitespaceArray = (value): string[] => {
  return [
    null,
    '',
    ' ',
