...
export const createTestConfig = (overrides: any = {}): Config => {
  ...
  return {
    ...
    dbAuthConnectionString: config.dbAuthConnectionString,
    isDev: config.isDev,
    isProd: config.isProd,
    vipJobUrl: 'https://non-existent-vip-environment.axtest.net/job',
    vipAuthHeader: 'Tenant-Auth none-existent-tenant-id-value',
    vipServiceBusSenderConnection:
      'EncryptedConnectionStringWithSenderPrivilege',
    vipServiceBusManagerConnection:
      'Endpoint=sb://NonEncryptedConnectionStringWithManagerPrivilege',
    ...overrides,
  };
};
